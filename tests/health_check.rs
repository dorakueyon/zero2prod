...
async fn spawn_app() -> TestApp {
    ...
    let connection_pool = configur_database(&configuration).await;
...
pub async fn configur_database(configuration: &Settings) -> PgPool {
    let mut connection =
        PgConnection::connect(&configuration.database.connecting_string_without_db())
            .await
